...
class TestStatementAnalyzerService:
    def test_analyze_new_file(self):
        ...
        uploaded_file_repo.find_by_hash.return_value = None
        uploaded_file_repo.save.return_value = {"id": str(uuid.uuid4())}
        ...
        uploaded_file_repo.find_by_hash.assert_called_once()
        uploaded_file_repo.save.assert_called_once()
    ...
    def test_analyze_duplicate_file(self):
        ...
        uploaded_file_repo = MagicMock()
        uploaded_file_repo.find_by_hash.return_value = existing_file
        ...
        assert result["uploaded_file_id"] == existing_file["id"]
        assert result["file_type"] == existing_file["file_type"]
        assert result["column_mapping"] == existing_file["column_mapping"]
        assert result["header_row_index"] == existing_file["header_row_index"]
        assert result["data_start_row_index"] == existing_file["data_start_row_index"]
        assert result["sample_data"] == existing_file["sample_data"]
