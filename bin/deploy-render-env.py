...
def main():
    ...
    api_key = get_nested_value(config, ["deployment", "render", "api_key"])
    service_id = get_nested_value(config, ["deployment", "render", "service_id"])

    if not api_key:
        print("Error: deployment.render.api_key not set in settings.prod.yaml")
        print("Get your API key from: Render Dashboard -> Account Settings -> API Keys")
        sys.exit(1)

    if not service_id:
        print("Error: deployment.render.service_id not set in settings.prod.yaml")
