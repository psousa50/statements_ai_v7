...
export const CreateEnhancementRuleModal: React.FC<CreateEnhancementRuleModalProps> = ({
  open,
  onClose,
  onSuccess,
}) => {
  const apiClient = useApi()
  const { createRule, loading, error } = useEnhancementRules()
  
  const [categories, setCategories] = useState<Category[]>([])
  const [counterpartyAccounts, setCounterpartyAccounts] = useState<CounterpartyAccount[]>([])
  const [formData, setFormData] = useState<EnhancementRuleCreate>({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error('Failed to create enhancement rule:', err)
    }
  }
  ...
  const handleFieldChange = (field: keyof EnhancementRuleCreate, value: any) => {
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      <DialogTitle>Create Enhancement Rule</DialogTitle>
      <DialogContent>
        {error && (
